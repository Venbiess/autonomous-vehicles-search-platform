...
class DrivingDojoPreprocessor(Preprocessor):
    ...
    def __init__(
        ...
        self._log(
            ...
            f"extract_archives={self.extract_archives}, repo_id={self.repo_id}, videos_dir={self.videos_dir}"
        )
        ...
        modes = {str(item.get("mode", "")) for item in self.episodes}
        self._log(
            f"[DrivingDojo] Ready: episodes={len(self.episodes)}, modes={sorted(modes)}, "
    ...
    def _prepare_sources(self) -> None:
        ...
        if not archives and self.download_from_hf:
    ...
    def _validate_archives_before_extract(self, archives: List[Path]) -> None:
        ...
        for archive_path in archives:
            ...
            self._recover_corrupted_archive(archive_path, expected_size=expected_size)
    ...
    def _extract_archives_if_needed(self, archives: List[Path]) -> None:
        ...
        for file_index, archive_path in enumerate(archives, start=1):
            ...
            try:
                ...
                self._recover_corrupted_archive(archive_path)
    ...
    def _recover_corrupted_archive(self, archive_path: Path, expected_size: Optional[int] = None) -> None:
            ...
            with open(archive_path, "wb") as fp:
                for chunk in response.iter_content(chunk_size=self.CHUNK_SIZE):
                    ...
                    if (now - log_last_at) >= self.LOG_PROGRESS_EVERY_SEC:
                        ...
                        self._log(
                            f"[DrivingDojo] Recovery download progress {Path(filename).name}: {percent:.1f}% "
        ...
        self._log(
            f"[DrivingDojo] Recovery download done: {archive_path.name} ({self._format_bytes(final_size)})"
        )
